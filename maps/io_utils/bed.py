#!/usr/bin/env python
"""
Support for reading, and intersection by chrom in BED format. 
Author: Yu Zhou
"""

import sys
from maps.io_utils.tabular import ParseError, TableRow, TableReader, Header
...
def count_bed_bases(fname):
    """Count total bases in BED file"""
    num = 0
    for rec in read_bed(fname):
        num += rec[2] - rec[1]
    return num
...
class Bed(TableRow):
    ...
    def find_middle(self):
        ...
        return self.get_start() + length/2
...
def split_by_strand(infile, outfile_f, outfile_r, clean=None):
    ...
    print >> sys.stderr, "Two files generated: %s, %s" % (outfile_f, outfile_r)
